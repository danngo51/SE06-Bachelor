...
interface FullMapMenuProps {
    featureProb: (date: string, weights?: { gru: number;xgboost: number }) => void;
...
const FullMapMenu = ({
    ...
    const [weightsPercentage, setWeightsPercentage] = useState({ gru: 0, xgboost: 100 });
    ...
    const convertToDecimalWeights = (percentageWeights: { gru: number; xgboost: number }) => {
        return {
    ...
    const handleWeightChange = (model: 'gru' | 'xgboost', newValue: number) => {
    ...
    const autoBalanceWeights = () => {
        ...
        if (currentTotal === 0) {
            ...
            setWeightsPercentage({ gru: 50, xgboost: 50 });
    ...
    const totalWeight = weightsPercentage.gru + weightsPercentage.xgboost;
