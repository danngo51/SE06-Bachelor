    ...
    if main_config_path.exists():
      ...
      try:
        ...
        if hybrid_model_path.exists():
            ...
            # Define the path changes
            replacements = [
                # Main country-specific file path
                (
                ...
                # Fallback path for when country-specific file is not found
                (
            ...
            for old_text, new_text in replacements:
                if old_text in hybrid_model_code:
                    hybrid_model_code = hybrid_model_code.replace(old_text, new_text)
                    print(f"✅ Updated path in hybrid_model.py: {old_text[:50]}...")
                else:
                    print(f"⚠️ Could not find exact match for path: {old_text[:50]}...")
        ...
        print(f"❌ Error updating hybrid_model.py: {e}")
