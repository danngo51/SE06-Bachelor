    ...
    def load_model(self, model_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if scaler_file.exists():
                ...
                # First check if the target scaler exists independently
                if target_scaler_file.exists():
                    print(f"Found target scaler: {target_scaler_file}")
                    self.scaler_target = joblib.load(str(target_scaler_file))
                
                # Check if features scaler exists
                if features_scaler_file.exists():
                    print(f"Found features scaler: {features_scaler_file}")
    ...
    def preprocess(self, data: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            if self.feature_cols is not None:
              ...
              # Scale features - handle feature mismatch
            if self.scaler_features is not None or self.scaler is not None:
                # Check if the feature dimensions match what the scaler expects
                scaler_to_use = self.scaler_features if self.scaler_features is not None else self.scaler
                
                expected_features = scaler_to_use.n_features_in_
                actual_features = features.shape[1]
                
                if expected_features != actual_features:
                    print(f"Feature mismatch: scaler expects {expected_features} features, got {actual_features}")
                    # Create a compatible array with the right number of features
                    compatible_features = np.zeros((features.shape[0], expected_features))
                    # Copy over as many features as possible
                    min_features = min(actual_features, expected_features)
                    compatible_features[:, :min_features] = features[:, :min_features]
        ...
        try:
            ...
            if self.scaler_target is not None:
                try:
                    ...
                    # Ensure we have the right shape for inverse transform (N, 1)
                    reshaped = flattened.reshape(-1, 1)
                    # Apply inverse transform
                    predictions = self.scaler_target.inverse_transform(reshaped).flatten()
                    # Reshape back to original shape if needed
                    if len(orig_shape) > 1:
                        predictions = predictions.reshape(orig_shape)
                except Exception as e:
                    print(f"Warning: Error in inverse transform: {e}")
                    # Fall back to original predictions if inverse transform fails
