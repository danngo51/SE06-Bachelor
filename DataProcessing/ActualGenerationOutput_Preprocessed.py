...
# Define folder paths
input_folder = "/Volumes/SSD/SEBachelor/entso-e data/ActualGenerationOutput/"  
output_folder = "/Volumes/SSD/SEBachelor/entso-e data/Preproccessed/Results/ActualGenerationOutput/"  
yearly_output_folder = "/Volumes/SSD/SEBachelor/entso-e data/Preproccessed/Results/ActualGenerationOutput/Yearly/"
full_output_file = "/Volumes/SSD/SEBachelor/entso-e data/Preproccessed/Results/ActualGenerationOutput/ActualGenerationOutput_FullDataset_2019_2023.csv"
...
for filename in sorted(os.listdir(input_folder)):
    if filename.endswith(".csv"):
        ...
        df_clean = df_clean.sort_values(by=["MapCode", "DateTime (UTC)"]).reset_index(drop=True)

        # **Step 5: Create lag features (grouped by MapCode)**
        """
        df_clean["lag_1h"] = df_clean.groupby("MapCode")["TotalLoadValue"].shift(1)
        df_clean["lag_24h"] = df_clean.groupby("MapCode")["TotalLoadValue"].shift(24)
        df_clean["lag_168h"] = df_clean.groupby("MapCode")["TotalLoadValue"].shift(168)
...
for year, dfs in yearly_data.items():
    ...
    yearly_file_path = os.path.join(yearly_output_folder, f"ActualGenerationOutput_{year}.csv")
